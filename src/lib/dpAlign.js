...
export function alignDP(scriptMsgs, rapUnits, opts = {}) {
  const {
    ...
    maxMergeUnits = 24, // 单条消息最多合并的 rap 单元数
  } = opts;
  ...
  for (let i = 1; i <= n; i++) {
    for (let j = 1; j <= m; j++) {
      // match: 支持一条脚本消息覆盖连续多个词级 ASR 单元，避免一段 Whisper
      // 结果包含多句说唱时只能一对一匹配而产生整体错位。
      let matchScore = -Infinity;
      let matchStart = j - 1;
      let matchSimilarity = 0;
      const maxSpan = Math.min(maxMergeUnits, j);
      for (let span = 1; span <= maxSpan; span++) {
        const start = j - span;
        const mergedText = rapUnits
          .slice(start, j)
          .map((unit) => unit.text || "")
          .join("");
        const similarity = fusedSimilarity(
          scriptMsgs[i - 1].text || scriptMsgs[i - 1].content || "",
          mergedText,
        );
        if (similarity >= threshold) {
          const candidate = dp[i - 1][start] + similarity;
  ...
  for (const op of operations) {
    if (op.op === "match") {
      const msgIdx = op.i - 1;
      const unitStart = op.spanStart ?? op.j - 1;
      const unitEnd = op.j - 1;
      const unit = rapUnits[unitStart];
      const lastUnit = rapUnits[unitEnd];
      const simScore = op.score;

      if (currentMatch && currentMatch.message_id === msgIdx) {
        // 继续合并到当前 match
        currentMatch.rap_span.end = round(lastUnit.end);
        currentMatch.rap_words_ref[1] = unitEnd;
        ...
        currentMatch = {
          message_id: msgIdx,
          rap_span: { start: round(unit.start), end: round(lastUnit.end) },
          match_type: classifyMatch(simScore),
          rap_words_ref: [unitStart, unitEnd],
          scores: [simScore],
          calibrated_confidence: 0,
...
function finalizeMatch(match, scriptMsgs, rapUnits, S, threshold, epsilon) {
  const avgScore =
    match.scores.reduce((a, b) => a + b, 0) / match.scores.length;
...
function calibrateConfidence(avgScore, maxScore) {
  ...
  if (combined >= 0.7) return round(0.9 + (combined - 0.7) * 0.33, 2); // 0.90-0.97
  if (combined >= 0.5) return round(0.65 + (combined - 0.5) * 1.25, 2); // 0.65-0.90
  if (combined >= 0.3) return round(0.35 + (combined - 0.3) * 1.5, 2); // 0.35-0.65
...
function classifyMatch(score) {
  ...
  if (score >= 0.3) return "paraphrase";
  return "partial";
}
...
function findProposedTime(msgIdx, scriptMsgs, rapUnits) {
  ...
  const totalDuration =
    rapUnits.length > 0 ? rapUnits[rapUnits.length - 1].end : 0;

  // 均匀分布的位置
  const estimated = totalDuration > 0 ? (msgIdx / n) * totalDuration : 0;
