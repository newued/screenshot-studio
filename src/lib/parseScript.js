...
export function parseScript(text) {
  ...
  return lines
    .map((line) => {
      ...
      const u = s[1].toUpperCase()
      const f = s[2]
      const h = f.match(RE.redpacket)
      if (h) return { type: 'redpacket', speaker: u, content: h[1] || '恭喜发财，大吉大利' }
      const B = f.match(RE.redpacketRecv)
      if (B) return { type: 'redpacketRecv', speaker: u, content: B[1] || '已领取' }
      const d = f.match(RE.transfer)
      if (d) return { type: 'transfer', speaker: u, amount: d[1], note: d[2] || '转账给朋友' }
      ...
      if (C) return { type: 'videoAnswered', speaker: u, duration: C[1] || '00:00' }
