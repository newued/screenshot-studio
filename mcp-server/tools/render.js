...
function setupRenderer(project, canvas, ctx, canvasChat) {
  async function loadImage(urlOrPath) {
    ...
    if (urlOrPath.startsWith('data:')) {
      try {
        const m = /^data:([^;]+);(base64|utf8|charset=utf-8),(.*)$/.exec(urlOrPath)
        let buf
        if (m && m[2] === 'base64') buf = Buffer.from(m[3], 'base64')
        else if (m && (m[2] === 'utf8' || m[2] === 'charset=utf-8')) buf = Buffer.from(decodeURIComponent(m[3]), 'utf8')
        else buf = Buffer.from(urlOrPath.split(',')[1], 'base64')
        return await napiLoadImage(buf)
      } catch { return null }
    }
    if (urlOrPath.startsWith('http://') || urlOrPath.startsWith('https://')) {
      try { const res = await fetch(urlOrPath); return napiLoadImage(Buffer.from(await res.arrayBuffer())) } catch { return null }
    }
    try { return await napiLoadImage(urlOrPath) } catch { return null }
  }
  ...
  return canvasChat.createChatFrameRenderer({
    ...
    canvas, ctx, loadImage, resolveAsset,
  })
}
...
function buildTiming(messages, duration) {
  ...
  const withSpan = messages.map((m, i) => {
    ...
    const ds = rap && rap.start != null ? +rap.start
      : (m.display_start != null ? +m.display_start
      : (m.start != null ? +m.start : null))
    const de = rap && rap.end != null ? +rap.end
      : (m.display_end != null ? +m.display_end
      : (m.end != null ? +m.end : null))
 ...
 */
export async function renderAndEncode(project, audioPath, outputPath, opts = {}) {
  ...
  const duration = project.duration || project.audioDuration || messages.reduce((max, m) => Math.max(max, m.display_end || 0), 10)
  ...
  ffmpegArgs.push('-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-b:v', '4M', '-movflags', '+faststart', '-shortest', outputPath)
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    if (i % 30 === 0) opts.onProgress?.({ frame: i, total: totalFrames, pct: Math.round(i / totalFrames * 100) })
  }
  ffmpeg.stdin.end()
  await once(ffmpeg, 'close')

  if (ffmpeg.exitCode !== 0) {
    throw new Error(`ffmpeg 编码失败 (code ${ffmpeg.exitCode}): ${stderr.slice(-500)}`)
  }
...
export async function renderFrame(frameIndex, fps, project) {
  ...
  const ctx = canvas.getContext('2d')
  const renderer = setupRenderer(project, canvas, ctx, canvasChat)
  const timing = buildTiming(project.messages || [], project.duration || project.audioDuration)
...
export async function exportStills(project, opts = {}) {
  ...
  const H = opts.height || 1920
  const messages = project.messages || []
  const duration = project.duration || project.audioDuration || messages.reduce((max, m) => Math.max(max, m.display_end || (m.rap_span?.end || 0)), 10)
  const timing = buildTiming(messages, duration)
  const outDir = opts.outputDir || join(os.homedir(), 'Downloads', 'screenshot-studio', 'export')
  ...
  const renderOne = async (t, name) => {
    ...
    await renderer.preload(messages, project.members || [])
    renderer.render(t, {
      messages, members: project.members || [],
      platform: project.platform || 'wechat', mode: project.mode || 'group',
      title: project.title || '', timing, duration, centered: !!project.centered,
    })
  ...
  if (opts.mode === 'slices') {
    ...
    const fullTiming = messages.map((m, i) => ({ ds: +(i * 0.01).toFixed(3), de: +duration.toFixed(3) }))
    const canvas = createCanvas(W, H)
    const ctx = canvas.getContext('2d')
    const renderer = setupRenderer(project, canvas, ctx, canvasChat)
    await renderer.preload(messages, project.members || [])
    renderer.render(Math.max(0.1, duration - 0.1), {
      messages, members: project.members || [],
      platform: project.platform || 'wechat', mode: project.mode || 'group',
      title: project.title || '', timing: fullTiming, duration,
      centered: !!project.centered, showAll: true,
    })
    const fullOut = join(outDir, 'full.png')
    await (await import('node:fs/promises')).writeFile(fullOut, canvas.toBuffer('image/png'))
    files.push(fullOut)
  }
