...
export async function transcribe(audioPath, scriptText, opts = {}) {
  ...
  if (!audioPath) {
    throw new Error('transcribe 需要 audioPath（音频文件绝对路径）。浏览器上传的 File 无法直接传给 MCP Server，需先保存到磁盘。')
  }
...
`
  ...
  try {
    await execFileAsync(py, [scriptPath], { timeout: 300_000 })
  } catch (err) {
    throw new Error(`faster-whisper 执行失败: ${err.message}`)
  } finally {
    // 清理临时文件
...
export function alignTimeline(rawSegments, scriptLines) {
  ...
  for (const line of scriptLines) {
    ...
    if (!lineChars.size) {
      // 纯标点行，跳过
      const prevEnd = results.length ? results[results.length - 1].display_end : 0
