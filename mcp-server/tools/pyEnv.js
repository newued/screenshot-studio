...
function candidatePythons() {
  ...
  if (process.platform === 'win32') {
    ...
    } catch { /* 探测异常则忽略，继续用通用候选 */ }
    list.push('C:\\Python311\\python.exe', 'C:\\Python312\\python.exe', 'C:\\Python313\\python.exe')
  }
...
export async function resolvePython(mod = 'librosa') {
  ...
  for (const py of candidatePythons()) {
    try {
      const { stdout } = await execFileAsync(py, ['-c', probe, mod], { timeout: 15_000 })
