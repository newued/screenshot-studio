...
const TOOLS = {
  ...
  render: async ({ project, audioPath, outputPath }) => {
    ...
    return await renderAndEncode(project, audioPath, outputPath, { onProgress: (p) => console.error(`[render] ${p.pct}%`) })
  },
  ...
  alignDP: async ({ audioPath, audioBase64, scriptText, model = loadAsrConfig().model, hopLength = 512 }) => {
    ...
    if (!resolvedAudioPath && audioBase64) {
      ...
      const ext = m ? (m[1].includes('mp4') ? 'm4a' : m[1].includes('ogg') ? 'ogg' : m[1].includes('wav') ? 'wav' : 'mp3') : 'mp3'
    ...
    if (!resolvedAudioPath) throw new Error('alignDP 需要 audioPath 或 audioBase64')
    ...
    try {
      ...
      try {
        ...
        asrSegments = raw
        rapUnits = raw.map(seg => ({ text: seg.text || '', start: seg.start || 0, end: seg.end || 0 }))
      ...
      const dpResult = runAlignDP(scriptMsgs, rapUnits, { gapPenalty: 0.15, threshold: 0.18, ambiguousEpsilon: 0.05, maxMergeUnits: 6 })
      ...
      return {
        beat_grid, grid_meta, onset_times, segments,
  ...
  aiReview: async ({ scriptText, beatGrid, rawSegments, mapping }) => {
...
const TOOL_DESCRIPTIONS = {
  ...
  beatGrid: '节拍网格提取：用 librosa 分析音频，输出 BPM、节拍时间数组、onset 包络、结构分段。',
  alignDP: '节拍网格+ASR联合DP对齐：先取节拍网格，再跑ASR（如可用），用有序DP全局对齐生成 mapping。',
  aiReview: 'AI语义对齐交接（说唱/演唱ASR漂移场景）：把 needs_review 条目+上下文整理成交接包。',
  aiApplyFix: 'AI写回：把 fixes 应用到 mapping 并重算 mapping_meta。',
  submitPage: '网页回调：用户确认页面信息并上传配音音频，写入 pipeline_state.json。',
  applyCreative: 'LLM 导演写回：把创意决策应用到对话，校验严格。',
  exportImage: '导出全图/切片（node 侧纯渲染）。',
  getWorkflow: '内置工作流剧本：首次会话调用即可获得完整流程食谱 + 脚本格式 + 表情库 + 动效枚举 + 曲风列表。',
