...
| Python   | 3.10+              | ASR 转写 / 节拍网格（librosa, soundfile, numpy, faster-whisper） | `agent-up` 自动探测并 `pip install`                   |
| ffmpeg   | 最新               | 视频编码 + 音频提取（**视频生成必需**）                          | `agent-up` 尝试 winget/brew/apt；缺失时会提示手动安装 |
| ffmpeg   | 最新               | 视频编码 + 音频提取（**视频生成必需**）                          | `agent-up` 自动探测；缺失时尝试 winget/brew/apt       |
