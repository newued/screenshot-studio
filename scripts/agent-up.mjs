...
function addRuntimePath(binDir) {
  if (!binDir || !existsSync(binDir)) return false;
  const entries = (process.env.PATH || "").split(delimiter);
  if (!entries.includes(binDir))
    process.env.PATH = [binDir, ...entries].filter(Boolean).join(delimiter);
  return true;
}
...
function locateFfmpeg() {
  const candidates = [
    ...
    "D:\\ffmpeg-9.0.1-full_build\\bin\\ffmpeg.exe",
  ];
  const found = candidates.find(
    (candidate) => candidate && existsSync(candidate),
  );
  if (found) addRuntimePath(dirname(found));
  return found;
}
...
async function installPython() {
  ...
  if (!runQuiet("winget --version").ok) return false;
  log("使用 winget 安装 Python 3.12...");
  return run(
...
async function ensurePythonDeps() {
  ...
  // 检测 Python 可用性
  let pyBin = await detectPython();
  if (!pyBin) {
    warn("未检测到 Python，尝试自动安装 3.12...");
    if (await installPython()) pyBin = await detectPython();
    if (!pyBin) {
  ...
  const methods = [
    {
      cmd: `${pyBin} -m pip install -i ${PIP_INDEX} ${missing.join(" ")}`,
      name: `${pyBin} -m pip (清华源)`,
    },
    {
      cmd: `pip install -i ${PIP_INDEX} ${missing.join(" ")}`,
      name: `pip (清华源)`,
    },
    {
      cmd: `pip3 install -i ${PIP_INDEX} ${missing.join(" ")}`,
      name: `pip3 (清华源)`,
    },
    // 回退：无镜像源
    {
      cmd: `${pyBin} -m pip install ${missing.join(" ")}`,
      name: `${pyBin} -m pip (官方源)`,
    },
  ];
  ...
  for (const m of methods) {
    ...
    if (run(m.cmd, { timeout: 300000 })) {
      ...
      const stillMissing = missing.filter(
        (pkg) => !runQuiet(`${pyBin} -c "import ${pkg}"`).ok,
      );
  ...
  if (process.platform === "win32" && process.env.LOCALAPPDATA) {
    for (const ver of ["Python313", "Python312", "Python311", "Python310"]) {
      ...
      if (existsSync(pythonPath)) {
        addRuntimePath(dirname(pythonPath));
        addRuntimePath(join(dirname(pythonPath), "Scripts"));
        candidates.unshift(pythonPath);
      }
    }
  }
...
function printManualInstallGuide(pyBin, missing) {
  ...
  console.log("若权限不足:");
  console.log(
    `  ${pyBin} -m pip install --user -i ${PIP_INDEX} ${missing.join(" ")}`,
  );
  console.log("");
...
async function main() {
  ...
  console.log("├────────────────────────────────────────┤");
  console.log(
    `│ ${ffmpegOk ? "✅" : "❌"} ffmpeg           ${ffmpegOk ? "已就绪" : "未安装 (视频功能不可用)"} │`,
  );
  console.log(
    `│ ${nodeOk ? "✅" : "❌"} Node 依赖         ${nodeOk ? "已就绪" : "安装失败"} │`,
  );
  console.log(
    `│ ${pythonOk ? "✅" : "❌"} Python 依赖      ${pythonOk ? "已就绪" : "安装失败 (ASR/对齐不可用)"} │`,
  );
  console.log("└────────────────────────────────────────┘");
  ...
  vite.unref();
  const mcp = spawn("node", ["mcp-server/index.js", "--http-only"], {
    cwd: ROOT,
    detached: true,
    stdio: "ignore",
    windowsHide: true,
  ...
  async function mcpHealth() {
    try {
      const res = await fetch("http://127.0.0.1:9527/api/health", {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if ((await mcpHealth()) && (await viteHealth())) {
      ok = true;
      break;
    }
  ...
  if (ok) {
    ...
    console.log("├────────────────────────────────────────┤");
    console.log(
      `│ 能力: ffmpeg ${cap(caps.ffmpeg)}  python ${cap(caps.python)} │`,
    );
    console.log(
      `│ 视频生成: ${caps.render ? "READY" : "NOT_READY（缺 ffmpeg/python，仅图片导出可用）"} │`,
    );
    ...
    const down = [!mcpUp && "mcp-server(9527)", !viteUp && "vite(5173)"]
      .filter(Boolean)
      .join("、");
  ...
  try {
    ...
    if (asrR.ok) ok(`ASR 模型 ${asrR.model} 已预载（~/.cache/huggingface）`);
      ...
      warn(
        `ASR 模型未预载[${asrR.reason}]: ${asrR.message}；离线时 alignDP 退化为 VAD/长度加权兜底（近似同步）`,
      );
  } catch (e) {
